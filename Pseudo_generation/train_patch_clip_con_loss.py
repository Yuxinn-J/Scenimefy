...
def train(args, loader, generator, generator_source, discriminator, g_optim, d_optim, g_ema, device, clip_model):
    save_dir = "/mnt/hdd/jyx/data/finetune-hosoda/FreezeG3_con001_patch005_clip1_crop32"
    ...
    g_nature.load_state_dict(ckpt["g_ema"], strict=False)

    # TODO
    vgg_model = '/mnt/ssd/jyx/stylegan2-pytorch/vgg19-dcbb9e9d.pth'
    ...
    # TODO double check
    if isinstance(generator, nn.parallel.DistributedDataParallel):
    ...
    for idx in pbar:
        ...
        # TODO: augmentatin
        augment_b4_clip = transforms.Compose([
        ...
        # TODO: generate corresponding images
        with torch.no_grad():
            g_nature.eval()
            nature_img, _ = g_nature(noise)

        # TODO: Extract content features from content image
        # the pre-trained VGG uses the input range of [0, 1]
        fake_feature = VGG((fake_img + 1 / 2.))
        nature_feature = VGG((nature_img + 1) / 2.)
        g_con_loss = mse_loss(fake_feature, nature_feature)

        # TODO: clip loss + patch-wise semantic consistency
        src_feat_single = clip_model.encode_image(clip_normalize(nature_img, device))
        ...
        clip_loss = (1.0 - sim(src_feat_single, fake_feat_single)).mean()
        args_size = 256
        crop_size = 32
        num_crop = 16
        src_img_crop, fake_img_crop = rand_sampling_mult(args_size, crop_size, num_crop, nature_img, fake_img)
        ...
        p_loss = lambda_patch * calc_patch(fake_patch, src_patch).mean()
        # c_loss = lambda_const * calc_const(fake_feat_single, src_feat_single)
        ...
        # print('=====/ng_loss: ', g_loss, 'vgg_loss: ', g_con_loss*0.01, 'clip_loss: '
        #      , c_loss, '===========', 'p_loss: ', p_loss)
        #print('=====/ng_loss: ', g_loss, 'vgg_loss: ', g_con_loss * 0.01, 'p_loss: ', 0.05*p_loss,
        #      'clip_loss: ', clip_loss)

        # TODO
        g_loss = g_loss + 0.01 * g_con_loss + 0.05 * p_loss + clip_loss
        # g_loss = g_loss + clip_loss
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", type=str, default='/mnt/ssd/jyx/stylegan2-pytorch/dataset/hosoda',
    ...
    parser.add_argument(
        "--ckpt",
        type=str,
        default='/mnt/ssd/jyx/Mask2Former/demo/220000.pt',
        #default='/mnt/hdd/jyx/data/stylegan2-church-config-f.pt',
    ...
    train(args, loader, generator, generator_source, discriminator, g_optim, d_optim, g_ema, device, clip_model)
